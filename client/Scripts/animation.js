...
function drawRoad(i) {
  ...
  for (
    y = 1;
    y <= Math.round((animationBoxWidht - actualRoadPartX) / roadPartSize);
    y++
  ) {
    if (i + y < t.length) {
      heightDifference = x1[i + y] * -pixelRation;
...
function myLoop(i) {
  setTimeout(function () {
    ...
    if (++i < t.length - 1)
      myLoop(i); //  increment i and call myLoop again if i > 0
    else {
      actualTime = 0;
      document.getElementById("playButton").disabled = false;
    }
